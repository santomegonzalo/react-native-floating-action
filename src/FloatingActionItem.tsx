...
interface IFloatingActionItemProps {
  tintColor: string;
  color: string;
  icon: any;
...
class FloatingActionItem extends Component<IFloatingActionItemProps> {
  constructor(props) {
    super(props);
  ...
  renderButton() {
    ...
    return (
      <View key="button" style={[styles.button, propStyles, shadow]}>
        {React.isValidElement(icon) ? icon : <Image style={[iconStyle, { tintColor: tintColor }]} source={icon} />}
      </View>
    );
  }

  render() {
    const { position, paddingTopBottom, render, margin, name, animated } = this.props;
